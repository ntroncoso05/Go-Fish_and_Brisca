...
namespace GoFish
{
    public class Player
    {
        ...
        {
            Name = name;
            PlayerObject = name.StartsWith("Opp") ? GameObject.Find($"{name}Area") : GameObject.Find(name);
        }
        ...
        public void GetNextHand(Deck stock)
        {
            ...
            while ((stock.Count() > 0) && (hand.Count() < 5))
            {
                ...
                cardToDeal.cardObject.GetComponent<CardMovement>()
                    .StartMoving(PlayerObject, cardToDeal.cardObject, speedDelayer);
                speedDelayer += 0.2f;
            }
        }
        ...
        public void AddCardsAndPullOutBooks(IEnumerable<Card> cards)
        {
            ...
            var foundBooks = hand.GroupBy(card => card.Value).Where(group => group.Count() == 4)
                .OrderBy(valueGroup => valueGroup.Key).Select(group => group.Key); // TODO - Crear book object
            ...
            {
                card.cardObject.GetComponent<CardMovement>().StartMoving(PlayerObject, card.cardObject, speedDelayer);
                speedDelayer += 0.2f;
            }

            if (foundBooks.Any())
                cards.ElementAt(0).cardObject.GetComponent<CardMovement>()
                    .RemoveCards(hand.Where(card => books.Contains(card.Value)).ToList(), PlayerObject);
        ...
        public void DrawCard(Deck stock)
        {
            if (stock.Count() > 0) 
            {
                AddCardsAndPullOutBooks(new List<Card>() { stock.Deal(stock.Count() - 1) });
            }
        }
